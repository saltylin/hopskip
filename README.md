...
owns the PTY sessions, runs the agent loop, hosts the MCP tools, and reads/writes a
...
- A POSIX shell (`$SHELL`, else `/bin/bash` or `/bin/sh`) — sessions run on
  daemon-owned PTYs; **no tmux needed**.
...
- **Browser terminal.** A real xterm.js terminal attached directly to a session's
  PTY (no tmux), so it behaves like a local terminal: native drag-select,
  double-click word-select across `/ : .`, wheel scrollback, selection survives
  scrolling, bar cursor, configurable scrollback. ⌘C / ⌘V copy & paste.
...
| Milestone | State |
|-----------|-------|
| PTY session core (`open`/`send_keys`/`read_screen`/`close`, done-sentinel; VT mirror) | ✅ |
...
| Browser terminal (live xterm.js over a direct PTY), chat transcript + saved chats | ✅ |
